...
const PedidoCRUD = () => {
  ...
  // Manejo de la eliminación de un pedido
const handleDelete = async (pedidoId) => {
  if (window.confirm('¿Estás segura de que deseas eliminar este pedido?')) {
    try {
      await deletePedido(pedidoId);
      const response = await getPedidos();
      setPedidos(response.data); // Actualiza la lista
      setError('');
    } catch (error) {
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '50px' }}>
      ...
      <ul>
        {pedidos.map((pedido) => (
          <li key={pedido.id} style={{ marginBottom: '10px' }}>
            ...
            <button class="btn btn-outline-info" onClick={() => handleEdit(pedido)}>Editar</button>
            <button class="btn btn-outline-danger" onClick={() => handleDelete(pedido.id)} > Eliminar </button>
            </div>
          </li>
        ))}
      </ul>
      ...
      <div style={{ marginTop: '10px' }}>
        {editMode ? (
        <button class="btn btn-outline-info" onClick={handleUpdate}>Actualizar Pedido</button>
      ) : (
        <button class="btn btn-outline-success" onClick={handleCreate}>Agregar Pedido</button>
      )}
      </div>
