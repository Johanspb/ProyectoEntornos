    ...
    <div style={{ textAlign: 'center', marginTop: '50px' }}>
      ...
      <ul>
        {articulos.map((articulo) => (
          <li key={articulo.id} style={{ marginBottom: '10px' }}>
            {articulo.nombre} - {articulo.descripcion} | Estado: {articulo.estado} | Impuesto: {articulo.impuesto}%
            <div>
              <button class="btn btn-outline-info" onClick={() => handleEdit(articulo)}>Editar</button>
            </div>
          </li>
        ))}
      </ul>
      ...
      <div style={{ marginTop: '10px' }}>
        <button class="btn btn-outline-success" onClick={editMode ? handleUpdate : handleCreate}>
